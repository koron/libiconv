    ...
    if (n >= 2) {
      if (wc < 0xe766) {
        ...
        if (wc < 0xe4c6) {
          ...
          r[0] = (unsigned char)(i < 6 ? i + 0xaa : i + 0xf2);
          return 2;
        } else {
          unsigned int i = wc - 0xe4c6;
          r[0] = (unsigned char)((i / 96) + 0xa1); i = i % 96;
          r[1] = (unsigned char)(i + (i >= 0x3f ? 0x41 : 0x40));
          return 2;
        }
      } else {
        /* User-defined characters, two-byte part of range U+E766..U+E864 */
        ...
        while (k1 < k2) {
          ...
          if (wc < gb18030_pua2charset[k*3+0])
            ...
            unsigned short c =
              (unsigned short)(gb18030_pua2charset[k*3+2] + (wc - gb18030_pua2charset[k*3+0]));
            r[0] = (c >> 8);
            r[1] = (c & 0xff);
            return 2;
          }
